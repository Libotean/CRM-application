<x-layout>
    <div class="container mx-auto">
        ...
        <div class="bg-gray-100 p-6 rounded-lg border border-gray-300 mb-8 shadow-inner">
            <form action="{{ route('consilier.clients.index') }}" method="GET" class="grid grid-cols-1 md:grid-cols-12 gap-4 items-end">
                <div class="md:col-span-4">
                    <label for="search" class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Cauta (Nume / Email)</label>
                    <div class="relative">
                        <input type="text" name="search" id="search" value="{{ request('search') }}" placeholder="Popescu..." class="w-full border-2 border-gray-300 rounded px-3 py-2 text-sm focus:border-black focus:ring-0 transition">
                    </div>
                </div>

                <div class="md:col-span-3">
                    <label for="status" class="block text-xs font-bold text-gray-500 uppercase tracking-wider mb-1">Status Cont</label>
                ...
                <div class="md:col-span-2 flex gap-2">
                    <button type="submit" class="w-full bg-black text-white border-2 border-black px-4 py-2 rounded text-sm font-bold hover:bg-gray-800 transition uppercase tracking-wide">Cauta</button>
                    @if(request()->anyFilled(['search', 'status']))
                        <a href="{{ route('consilier.clients.index') }}" class="flex items-center justify-center w-12 bg-white text-red-700 border-2 border-red-200 hover:border-red-700 hover:bg-red-50 rounded transition">✕</a>
                    @endif
                </div>
            </form>
        </div>
        ...
        <div class="bg-white shadow-lg rounded-lg overflow-hidden border border-gray-200">
            <table class="min-w-full divide-y divide-gray-200">
                ...
                @foreach ($clients as $client)
                    <tr class="hover:bg-gray-50 transition duration-150">
                        ...
                        <td class="py-4 px-6 whitespace-nowrap text-gray-600">{{ $client->email }}</td>
                        <td class="py-4 px-6 whitespace-nowrap">
                            @if ($client->status == 'activ')
                                <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-green-100 text-green-800"><span class="w-2 h-2 bg-green-500 rounded-full mr-1.5"></span> Activ</span>
                            @else
                        ...
                        <td class="py-4 px-6 whitespace-nowrap text-right">
                            <div class="flex items-center justify-end gap-3">
                                <a href="{{ route('consilier.clients.show', $client->id ) }}" class="text-sm font-bold text-red-700 hover:text-black transition uppercase tracking-wide mr-2">Detalii</a>
                                <a href="{{ route('consilier.clients.edit', $client->id ) }}" class="text-gray-400 hover:text-blue-600 transition p-1"><svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M15.232 5.232l3.536 3.536m-2.036-5.036a2.5 2.5 0 113.536 3.536L6.5 21.036H3v-3.572L16.732 3.732z"></path></svg></a>
                            </div>
                        </td>
                    </tr>
                @endforeach
            ...
            @if($clients->isEmpty())
                <div class="p-8 text-center text-gray-500">Nu exista niciun client creat.</div>
            @endif
        </div>
    </div>

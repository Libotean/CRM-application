<x-layout>
    <div class="w-full px-6 py-6 bg-gray-50 min-h-screen">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start">
            ...
            {{-- ================= ZONA PRINCIPALA (COL 9) ================= --}}
            <div class="lg:col-span-9 space-y-8">

                {{-- 1. FORMULAR TEST DRIVE --}}
                <div class="bg-white p-8 rounded-2xl shadow-xl border border-gray-100">
                    <div class="flex items-center gap-3 mb-6 pb-4 border-b border-gray-100">
                        <div class="bg-black text-white w-10 h-10 flex items-center justify-center rounded-full shadow-md">
                    ...
                    <form action="{{ route('test_drives.store', $client->id) }}" method="POST">
                        ...
                        <div class="space-y-6">
                            ...
                            <div>
                                ...
                                @if($client->vehicles->isNotEmpty())
                                    ...
                                    <div class="w-full border border-gray-200 bg-gray-100 rounded-lg px-4 py-3 text-gray-700 font-bold flex items-center justify-between shadow-sm">
                                        <div class="flex items-center gap-3">
                                            {{-- Am scos complet iconita cu sageata care era aici --}}
                                            <span>
                                                {{ $assignedCar->make->name ?? 'Marca' }} {{ $assignedCar->model->name ?? 'Model' }}
                                                <span class="text-gray-500 font-normal text-sm ml-1">- {{ $assignedCar->vin ?? 'Fara VIN' }}</span>
                                            </span>
                                        </div>
                                ...
                                @else
                                    {{-- Fallback --}}
                                    <div class="p-4 bg-red-50 text-red-700 rounded-lg border border-red-200 flex items-center">
                                        <svg class="w-5 h-5 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 8v4m0 4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z"></path></svg>
                                         Acest client nu are niciun vehicul asignat!
                                    </div>
                                @endif
                            </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                                <div>
                                    <label class="block text-xs font-bold text-gray-400 uppercase mb-2">Data Programarii</label>
                                    <input type="date" name="scheduled_date" value="{{ date('Y-m-d') }}"
                                           class="w-full border border-gray-200 rounded-lg px-4 py-3 text-gray-700 focus:outline-none focus:border-red-700 focus:ring-1 focus:ring-red-700 transition">
                                </div>

                                <div>
                                    <label class="block text-xs font-bold text-gray-400 uppercase mb-2">Ora</label>
                                    <input type="time" name="scheduled_time" value="{{ date('H:00') }}"
                                           class="w-full border border-gray-200 rounded-lg px-4 py-3 text-gray-700 focus:outline-none focus:border-red-700 focus:ring-1 focus:ring-red-700 transition">
                                </div>
                            </div>
                            ...
                            <div>
                                ...
                                <div class="relative">
                                    <input type="number" name="start_km" placeholder="Ex: 15000"
                                           class="w-full border border-gray-200 rounded-lg px-4 py-3 pl-14 text-gray-700 focus:outline-none focus:border-red-700 focus:ring-1 focus:ring-red-700 transition">
                                    <div class="absolute inset-y-0 left-0 pl-4 flex items-center pointer-events-none">
                                        <span class="text-gray-500 text-sm font-bold border-r border-gray-300 pr-3">KM</span>
                                    </div>
                                </div>
                            </div>

                            {{-- Buton Salvare (Mic si in Dreapta) --}}
                            <div class="pt-4 flex justify-end">
                                <button type="submit" class="bg-red-700 text-white font-bold text-sm px-8 py-3 rounded-lg shadow-md hover:bg-red-800 hover:shadow-lg transition transform active:scale-[0.99] flex items-center gap-2">
                ...
                {{-- 2. FORMULAR JURNAL INTERACTIUNE --}}
                <div class="bg-white p-8 rounded-2xl shadow-lg border border-gray-200">
                    ...
                    <form action="{{ route('consilier.leads.store', $client->id) }}" method="POST">
                        ...
                        <div class="flex flex-col md:flex-row gap-4 mb-4">
                            <div class="flex-1">
                                ...
                                <div class="flex gap-2">
                                    <input type="date" name="appointment_date" value="{{ date('Y-m-d') }}" class="w-full border border-gray-300 rounded px-3 py-2 text-sm focus:border-black focus:ring-1 focus:ring-black">
                                    <input type="time" name="appointment_time" value="{{ date('H:00') }}" class="w-32 border border-gray-300 rounded px-3 py-2 text-sm focus:border-black focus:ring-1 focus:ring-black">
                                </div>
                            </div>
                            <div class="flex-1">
                                <label class="block text-xs font-bold text-gray-400 uppercase mb-1">Metoda</label>
                                <select name="method" class="w-full border border-gray-300 rounded px-3 py-2 text-sm bg-white focus:border-black focus:ring-1 focus:ring-black">
                            ...
                            <div class="flex-1">
                                ...
                                <select name="objective" class="w-full border border-gray-300 rounded px-3 py-2 text-sm bg-white focus:border-black focus:ring-1 focus:ring-black">
                        ...
                        <div class="flex gap-4 items-start">
                            <div class="flex-grow">
                                <textarea name="notes" rows="1" class="w-full border border-gray-300 rounded px-3 py-3 text-sm resize-none focus:border-black focus:ring-1 focus:ring-black" placeholder="Scrie detalii..."></textarea>
                            </div>
                            <button type="submit" class="bg-gray-800 text-white px-8 py-3 rounded text-sm font-bold uppercase hover:bg-black transition shadow-lg flex-shrink-0">
                                Adauga
                            </button>
                        </div>
                    </form>
                                        ...
                                        <div class="flex justify-between items-start mb-3">
                                            ...
                                            <form action="{{ route('consilier.leads.toggle', $lead->id) }}" method="POST">
                                                ...
                                                <button type="submit" class="text-xs font-bold px-4 py-2 rounded border transition flex items-center gap-2 {{ $lead->is_completed ? 'bg-green-50 text-green-700 border-green-200 hover:bg-green-100' : 'bg-white text-yellow-700 border-yellow-300 hover:bg-yellow-50 shadow-sm' }}">
                                                    @if($lead->is_completed) FINALIZAT @else IN ASTEPTARE @endif
                                                </button>
                                            </form>
                                        </div>
                                        @if($lead->notes)
                                            <div class="text-gray-700 text-sm leading-relaxed border-t border-gray-100 pt-3 mt-2">{{ $lead->notes }}</div>
                                        @else
